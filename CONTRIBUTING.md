...
[Txt Edit](https://grayface.github.io/mm/#Txt-Edit).
...
pipeline, verify `mm678 build --no-release` still succeeds before and
after, and that the output under `build/` is unchanged unless the change is
intentional.

## 中文贡献者

直接用 Poedit 编辑 `translations/zh_CN/mm678.po`（简体）；繁体由
