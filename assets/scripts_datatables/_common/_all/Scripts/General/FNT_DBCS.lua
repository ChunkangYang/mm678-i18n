...
local activeFontAddr -- font currently in DBCS mode
local cachedSpaceWidth = {} -- {fontAddr = spaceWidth} table
local cachedCharShape = {} -- {fontAddr = charShape} table
local highByte = 0
local lowByte = 0
local lastByte = 0
local hiddenByte -- byte whose width/space metrics are currently zeroed in activeFontAddr
local dbcsFnts = {}
local cachedWidth
...
local function logError(message)
	message = "[FNT_DBCS] " .. tostring(message)
	local logFunc = rawget(_G, "Log")
	local mergeTable = rawget(_G, "Merge")
	if logFunc ~= nil and type(mergeTable) == "table" and mergeTable.Log ~= nil then
		pcall(logFunc, mergeTable.Log.Error, "%s", message)
	else
		pcall(print, message)
	end
end
...
-- a missing or bad .fnt must abort DBCS rendering (see the pcall around the
-- char-test hook) instead of letting FNT getters read arbitrary memory
...
function DBCS.loadDbcsFnt(heightAndDecoStr, highByte)
	...
	local dbFntAddr = dbcsFnts[heightAndDecoStr][highByte]
	if dbFntAddr ~= nil and fontLooksReadable(dbFntAddr) then
		return dbFntAddr
	end
	dbcsFnts[heightAndDecoStr][highByte] = nil -- never keep a failed load in the cache
	local fileName = "DBCS_" .. heightAndDecoStr .. "_" .. string.format("%02X", highByte) .. ".fnt"
	-- probe first where possible: the engine's own missing-file handling is fatal
	if Game.CanLoadFileFromLod ~= nil and not Game.CanLoadFileFromLod(fileName) then
		error(fileName .. " not found")
	end
	dbFntAddr = Game.LoadDataFileFromLod(fileName)
	if not fontLooksReadable(dbFntAddr) then
		error(fileName .. " is missing or not a valid font")
	end
...
function DBCS.truncate(str, encoding)
	...
	local pos = lastIndexOf(str, "\15") or 0
...
-- ============ DBCS RENDERING STATE HELPERS START ============
-- file-level (not inside the hook proc) so that the pcall error handler
-- in dbcsProc can restore font state after a failure

local function restoreHiddenByteInFont(fontAddr)
	if hiddenByte ~= nil and cachedWidth ~= nil then
		FNT.setCharSpaceBefore(fontAddr, hiddenByte, cachedSpaceBefore)
		FNT.setCharSpaceAfter(fontAddr, hiddenByte, cachedSpaceAfter)
		FNT.setCharWidth(fontAddr, hiddenByte, cachedWidth)
	end
	hiddenByte = nil
end

local function emptyAndCacheByteInFont(fontAddr, byte)
...
local function switchOnDbcsMode(fontAddr)
...
local function switchOffDbcsMode(fontAddr) -- restore hidden byte metrics, space and cachedCharShape
	fontAddr = fontAddr or activeFontAddr
	if fontAddr ~= nil then
		-- pcall so that one failed restore can't prevent the state reset below
		pcall(restoreHiddenByteInFont, fontAddr)
...
-- ============ DBCS RENDERING STATE HELPERS END ============


local function dbcsProcUnsafe(d)
	...
	-- the game can switch fonts between two drawn characters;
	-- never carry a half-read DBCS sequence from one font into another
	if dbcsMode and activeFontAddr ~= nil and activeFontAddr ~= fontAddr then
		switchOffDbcsMode(activeFontAddr)
	end
...
-- before using the script
	...
	elseif dbcsMode == true then
		...
		elseif byte == 7 and lastByteCheckAndCache(byte) then -- [BEL]
			if highByte == 0 or hiddenByte ~= lowByte then -- malformed sequence, e.g. [SO] directly followed by [BEL]
				switchOffDbcsMode(fontAddr)
				return
			end
			...
			restoreHiddenByteInFont(fontAddr) -- restore the low byte's metrics
			highByte = 0
			lowByte = 0
		elseif highByte == 0 then -- high byte
			...
			elseif lastByteCheckAndCache(301) then
				emptyAndCacheByteInFont(fontAddr, byte)
				highByte = byte
			end
		elseif lastByteCheckAndCache(300) then -- low byte
			restoreHiddenByteInFont(fontAddr) -- restore the high byte's metrics
			emptyAndCacheByteInFont(fontAddr, byte)
			lowByte = byte
		end
	end
...
local function dbcsProc(d)
	local ok, err = pcall(dbcsProcUnsafe, d)
	if not ok then
		switchOffDbcsMode()
		err = tostring(err)
		if err ~= lastReportedError then
			lastReportedError = err
			logError("recovered from a DBCS rendering error: " .. err)
		end
	end
end
...
local function GetFromFile(fname, n, col)
	local t = {}
...
local function getRosterTxtInfo(index, infoType) -- infoType can be "Biography" or anything else (which means Name)
	local col = 2
